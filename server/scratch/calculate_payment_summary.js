...
// Mocking helper functions from Accounts.jsx
...
async function run() {
  try {
    ...
    const [accRes, salesRes, invRes, supRes, salRes, rentRes, otherRes, expRes] = await Promise.all([
      ...
      fetch("https://erp-backend-3rf8.onrender.com/api/sales", { headers: h }),
    ...
    console.log("Filtered General Expenses IDs:", filteredGeneralExpenses.map(e => ({ id: e.id, type: e.expense_type, payment: e.payment_type, amount: e.amount })));

    // Reconstruct paymentSummary logic
    const rawList = [
      ...filteredSales, 
    ...
    console.log("\n--- UBL accounts trace ---");
    console.log(JSON.stringify(transactionTrace, null, 2));
