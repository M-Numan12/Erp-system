...
async function run() {
  try {
    ...
    console.log("Tracing cash balance step-by-step WITHOUT clamping:");
    ...
    rawList.forEach((t, idx) => {
      ...
      if (bal < 0 || oldBal < 0) {
        console.log(`[Idx ${idx}] ID: ${t.id}, Type: ${t.isSale ? 'Sale' : (t.isExpense ? (t.isTransportFare ? 'Fare' : 'Expense') : 'Income')}, Purchase?: ${t.supplier_id !== undefined}, Amount: ${amt}, Bal: ${oldBal} -> ${bal}, Date: ${t.date.toISOString().split('T')[0]}`);
      }
    });
